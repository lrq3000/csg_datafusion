...
* **Databases realignment** and joining using relational databases rules (the toolset assumes only two fields are present: name and final_diagnosis, all other fields are dynamically accounted for). This is similar to the rationale that motivated the [tidy data paper by Wickham](https://doi.org/10.18637/jss.v059.i10) (see also [this](http://www.jeannicholashould.com/tidy-data-in-python.html) and [this](https://pystitch.github.io/_downloads/tidy.pdf)), although the author did not know of it at the time, so this toolset likely is tidy compatible, accepts untidy data as input and then outputs tidy-like data.
...
* [OpenRefine](https://github.com/OpenRefine/OpenRefine). This Python app was made years after `csg_datafusion`, but it generalizes the data alignment and data validation parts with its GREL language to program custom rules. If I had to redo this project, I would recommend to use this module for these steps.
* [docling](https://github.com/DS4SD/docling). Also published years later. Docling allows to convert office documents into text or markdown, including with OCR if necessary. This would have been greatly useful for the data extraction from reports.
* LLMs and GPTs: also published years later obviously, they would have been very useful to replace regular expressions to extract the sub diagnoses and final diagnoses out of the reports in a more robust way.

In summary, `csg_datafusion` was a precursor in datafusion before the advent of more sophisticated tools to generalize the process, hence why everything was done "by hand", coded from the ground up. What remains to be available in a more general package is the automatic extraction of DICOMs metadata, and alignment between DICOM files, NIFTIs, and demographics, as well as the unified database querying system for retrospective dataset creation based on flexible search criteria.
